...
def run(args):
    ...
    agent = agents.__dict__[args.agent_type].__dict__[args.agent_name](agent_config, args)
    print(agent.model)
    ...
    if args.offline_training:  # Non-incremental learning / offline_training / measure the upper-bound performance
        ...
        for i in range(len(task_names)):
            ...
            for n, p in agent.model.named_parameters():
                if p.requires_grad:
                    O = agent.regularization_terms[1]['importance'][n] #Omega
...
def get_args(argv):
    ...
    parser.add_argument('--model_name', type=str, default='MLP1000_drop', help="The name of actual model for the backbone")
    ...
    parser.add_argument('--agent_name', type=str, default='MAS', help="The class name of agent")
    ...
    #Regularization
    parser.add_argument('--reg_coef', nargs="+", type=float, default=[0.01], help="The coefficient for regularization. Larger means less plasilicity. Give a list for hyperparameter search.")
    parser.add_argument('--online_reg', type=bool, default=True, help="Flag for online regularization computation")

    #Added for SIM
    # parser.add_argument('--SIM', type=bool, default = True, "SIM flag")
