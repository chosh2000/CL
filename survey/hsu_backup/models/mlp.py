...
class MLP_drop(nn.Module):
    def __init__(self, out_dim=10, in_channel=1, img_sz=32, hidden_dim=256):
        ...
        self.mask1 = torch.ones(hidden_dim)
        self.mask2 = torch.ones(hidden_dim)
    ...
    def forward(self, x):
        ...
        x = F.relu(nn.Linear(self.in_dim, hidden_dim))
        x = x*self.mask1
        x = F.relu(nn.Linear(self.hidden_dim, hidden_dim))
        x = x*self.mask2
        x = self.logits(x)
        return x
